...
def test_health_shape(client):
    ...
    assert "provider" in data  # groq or ollama depending on config


def test_health_ollama_reachable(client):
    data = client.get("/health").json()
    if data.get("provider") != "ollama":
        pytest.skip("Ollama not the active provider in this environment")
    assert data["ollama"] != "unreachable", "Ollama is not reachable — is it running?"
...
def test_mcp_tools_includes_workspace_tools(client):
    ...
    assert "write_workspace_file" in names, (
        "write_workspace_file missing from MCP tools"
    )
...
def test_chat_read_jenkinsfile(client):
    ...
    r = client.post(
        "/chat", json={"message": "read the Jenkinsfile and tell me the stages"}
    )
    assert r.status_code == 200
    data = r.json()
    assert data["type"] in ("pending_approval", "auto_executed", "message")
